...
class SisowService
{
	...
	public function setMerchant($merchantid, $merchantkey, $shopid = 0) {
	...
	// RefundRequest
	public function RefundRequest($trxid, $amount) {
		$this->amount = $amount;
		
		$pars = array();
		$pars["merchantid"] = $this->merchantId;
		$pars["trxid"] = $trxid;
		$pars["amount"] = $amount * 100.0;
	...
	public function AdjustPurchaseId($trxId, $oldId, $newId){
		$pars = array();
		$pars["merchantid"] = $this->merchantId;
		$pars["trxid"] = $trxId;
		$pars["old"] = $oldId;
		$pars["new"] = $newId;
		$pars["sha1"] = sha1($trxId . $oldId . $newId . $this->merchantId . $this->merchantKey);

		if (!$this->send("AdjustPurchaseId", $pars))
			return -1;
		$this->purchaseId = $this->parse("purchaseid");
		if (!$this->purchaseId) {
			$this->error();
